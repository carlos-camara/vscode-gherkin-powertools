...
- **Create empty step definition**: When an undefined step is detected (⚠️), this action automatically generates a safe, syntax-compliant Python stub (`@given(...)`) and inserts it into your `steps/` folder. It intelligently escapes strings containing quotes or emojis, guarantees collision-free function names (e.g. `def step_impl_1(context)`), and resolves `And` and `But` keywords by scanning upwards to inherit the correct preceding decorator.
> [!TIP]
