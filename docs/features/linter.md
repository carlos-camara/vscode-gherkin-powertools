...
2. **Cascading Error Suppression**: When the official parser crashes due to a structural error (e.g., missing a `:` after `Scenario`), it traditionally breaks the internal state machine, causing a massive "wall of red squiggles" on perfectly valid steps below the error. The linter now intelligently suppresses these cascading syntax errors on locally valid lines, pinpointing exactly where the true structural error occurred without overwhelming the user.
